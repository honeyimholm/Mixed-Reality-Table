...
class Player(pygame.sprite.Sprite):
    ...
    def change_angle(self, new_angle):
        if(self.curr_angle!=new_angle):
            self.image = pygame.transform.rotate(self.image,-self.curr_angle)
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            self.last_shot = now
            if self.power == 1:
                bullet = Bullet(self.rect.centerx, self.rect.top, self.curr_angle)
                all_sprites.add(bullet)
                bullets.add(bullet)
                shooting_sound.play()
            if self.power == 2:
                bullet1 = Bullet(self.rect.left, self.rect.centery, self.curr_angle)
                bullet2 = Bullet(self.rect.right, self.rect.centery, self.curr_angle)
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, curr_angle):
        pygame.sprite.Sprite.__init__(self)
        self.image = bullet_img
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
        ## place the bullet according to the current position of the player
        #setting current angle to that of the players
        self.curr_angle = curr_angle
        if(self.curr_angle!=0):
            self.image = pygame.transform.rotate(self.image,curr_angle)
        #now set speed and offset
        if(curr_angle==0):
            self.rect.bottom = y 
            self.rect.centerx = x
            self.speedy = -10
...
with open(pipe_dir) as fifo:
    #have game continuously running for demo
    while True:
