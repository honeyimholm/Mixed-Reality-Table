...
def newmob():
    ...
    while(not valid_spawn):
        ...
        mob_element = Mob()
        for m in mobs:
            if(mob_element.rect.colliderect(m.rect)):
                valid_spawn = False
                mob_element.kill()
                break
...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.speedy = random.randrange(-4, 4)        ## for randomizing the speed of the Mob
...
for image in obstacle_list:
    obstacle_images.append(pygame.image.load(path.join(img_dir, image)).convert())
...
while running:
    if menu_display:
        ...
        pygame.mixer.music.load(path.join(sound_folder, 'humble.ogg'))
    ...
    #########################
        #check if emoji collides with another emoji
    #custom check for collision - react with bounce
    #can't enumerate mobs since hashed...need to supress duplicate collisions
    mob_list = []
    for m in mobs.sprites():
        mob_list.append(m)
    #mob_list = mobs.sprites()[:]
    for obj1 in mob_list:
        for obj2 in mob_list:
            if obj1!=obj2:
                if obj1.rect.colliderect(obj2.rect):
                    if(obj1.rect.width-10>obj1.rect.x-obj2.rect.x and obj1.rect.width-10<obj2.rect.x-obj1.rect.x):
                        obj1.speedx=random.randrange(-5, -1)
                    elif(obj1.rect.width-10>obj2.rect.x-obj1.rect.x):
                        obj1.speedx=random.randrange(1, 5)
                    if(obj1.rect.height-10<obj1.rect.y-obj2.rect.y and obj1.rect.height-10>obj2.rect.y-obj1.rect.y):
                        obj1.speedy= random.randrange(1, 5)
                        obj1.speedx= random.randrange(1, 5)
                    elif(obj1.rect.height-10<obj2.rect.y-obj1.rect.y):
                        obj1.speedy=random.randrange(-5, -1)
                        obj1.speedx=random.randrange(-5, -1)

    #
    #hits = pygame.sprite.spritecollide(mobs, mobs, False, pygame.sprite.collide_rect)        ## gives back a list, True makes the mob element disappear
